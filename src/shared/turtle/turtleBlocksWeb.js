...
const turtleFunctions = ({
  manager = {}, workerPoolObj = {}, aceObject = {},
}) => {
  const managerObj = manager;
  const poolObj = workerPoolObj;
  const ace = aceObject;

  managerObj.blockColors = (name) => {
    const mapper = {
  ...
  managerObj.initiateRunCode = () => {
    ...
    const code = Blockly.Python.workspaceToCode(managerObj.workspace);
    ...
    managerObj.runCode(code, 'userCanvas', true, frames, delay, true).then(() => {
      ...
      const request = {
        ...
        xmlWorkSpace: Blockly.Xml.domToText(Blockly.Xml.workspaceToDom(managerObj.workspace)),
        validated,
      };
      const msg = {
        type: 'validateQuestion',
  ...
  managerObj.initializeBlockly = (response) => {
    try {
      ...
      if (xmlTurtleCategoryElement) {
        xmlTurtleCategoryElement.setAttribute('colour', managerObj.blockColors('turtle').category);
      }
      const workspace = Blockly.inject('turtleBlock', {
        ...
        horizontalLayout: (window.innerWidth < 641 && !xmlBlock.includes('category')),
        maxBlocks,
      });
      ...
      workspace.addChangeListener(managerObj.handleBlocksChange);
      managerObj.workspace = workspace;
      // const msg = {
      //   type: 'turtle_import',
      ...
      if (window.ReactNativeWebView) {
        window.ReactNativeWebView.postMessage(`Blockly error:  ${error}`);
      } else {
