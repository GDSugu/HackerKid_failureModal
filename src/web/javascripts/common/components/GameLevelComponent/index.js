...
const GameLevelButton = ({
  ...
  forCodekata = false,
    ...
    <button id={`turtle-${virtualId}`} className={`btn game-level-button ${question.state || 'open'} ${isCurrentQuestion ? 'current-question' : ''}`}
    onClick={() => {
      if (forCodekata) {
...
const GameLevelComponent = ({ gameData, handleFetchQuestion, forCodekata = false }, ref) => {
  const { state: { device } } = useRootPageState();

  const isCurrentQuestion = forCodekata ? (gameData.questionList
          ...
          && <>
            <div className="close-btn-mob btn" onClick={closeLevelComponent}>
              <d className="d-flex align-items-center justify-content-between w-100">
                ...
                {forCodekata ? <svg width="16" height="19" viewBox="0 0 16 19" fill="none" xmlns="http://www.w3.org/2000/svg">
