...
import { getSession } from '../../../../hooks/common/framework';
...
const LeaderBoardCard = ({ data, className }) => <>
  { data && <>
        ...
        <div className="leaderboard-card-content">
          ...
          }
          {
            data.leaderBoardUserData && <>
              ...
              <p>
                <span>{`#${data.leaderBoardUserData.rank}`}</span>
                <span>
                  <FormattedMessage
                    defaultMessage={'rank'}
                    description={'rank'}
            ...
            <div className="sideboard-content-data">
              {
                data.leaderboardData && data.leaderboardData.slice(0, 3).map((item, index) => <div key={index} className={`d-flex align-items-center justify-content-between ${data.leaderBoardUserData.rank === item.rank ? 'font-weight-bold' : ''}`}>
...
const Dashboard = () => {
  ...
  let overAllQuestionsCount = 0;
  let validSubmissionsCount = 0;
  let totalPointsEarned = 0;

  Object.keys(dashBoardData).forEach((key) => {
    overAllQuestionsCount += dashBoardData[key].overAllQuestionCount;
    validSubmissionsCount += dashBoardData[key].validSubmissionCount;
  ...
  populateScore('#yourScore', validSubmissionsCount, parseInt((validSubmissionsCount / overAllQuestionsCount) * 100, 10));
  ...
  return <>
          ...
          && <>
            <div className="hero-data-content">
              <div className="hero-data">
                ...
                <p className='mb-0'>
                  <FormattedMessage
                    defaultMessage={`${12345} coins`}
                    description={'hk coins'}
                  />
                </p>
              </div>
              <div className="hero-data">
      ...
      <div className="dashboard-body row">
            ...
            <div className="profile-content card card-block">
              {
                dashboardUserData
                && <>
                  {/* <p className="profile-content-heading">{userData.name}</p> */}
                  <p className="profile-content-heading">{session.name}</p>
        ...
        </div>
        {
          ...
          <div className="col-md-4 dashboard-sideboard">
            <LeaderBoardCard data={{ leaderboardData, leaderBoardUserData }} />
          </div>
          </>
        }
            ...
            <div className="challenges-block">
              <div className="row no-gutters">
                ...
                }
                {
                  !dashboardUserData
                  && <>
                    <div className="d-flex align-items-center justify-content-center col py-5">
                      <p className='mb-0'>
                        <FormattedMessage
                          defaultMessage={'Loading...'}
                          description={'Loading...'}
    ...
    <BottomSheet id='viewMoreSheet'>
      <LeaderBoardCard data={{ leaderboardData, leaderBoardUserData }} className='sheet-card' />
    </BottomSheet>
    {
